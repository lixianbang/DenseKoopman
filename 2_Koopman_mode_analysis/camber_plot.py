...
# 初始化速度记录矩阵
data_vel = np.zeros((all_x.shape[0], all_x.shape[1]))  # ID Frame vel_value
...
# 绘制速度分布曲面图   使用data_vel   [id, time]
def camber_3d_vel():

    x_ = np.arange(all_x.shape[0])  # id
    y_ = np.arange(all_x.shape[1])  # time
    ...
    ax3d.set_title(f'Velocity Time-Space Graph of mode {split_num+1}')
    ax3d.set_xlabel('ID')
