...
class MID():
            ...
            if epoch % self.config.eval_every == 0:
                ...
                for i, scene in enumerate(self.eval_scenes):
                    ...
                    batch = get_timesteps_data(env=self.eval_env, scene=scene, t=timesteps, node_type=node_type, state=self.hyperparams['state'],
                                   ...
                                   min_ht=7, max_ht=7, min_ft=12, max_ft=12, hyperparams=self.hyperparams)
                    if batch is None:
                        continue
                    test_batch = batch[0]
                    nodes = batch[1]
    ...
    def eval(self):
        ...
        # 循环每一个txt
        predictions_all = np.zeros((20, 20, 12, 2))
        for i, scene in enumerate(self.eval_scenes):
            print(f"----- Evaluating Scene {i + 1}/{len(self.eval_scenes)}")
            timesteps = np.arange(0, 20)
            ...
            nodes = batch[1]
            timesteps_o = batch[2]
            traj_pred = self.model.generate(test_batch, node_type, num_points=12, sample=20,
                                            bestof=True)  # B * 20 * 12 * 2

            predictions_all += traj_pred
        predictions_dict = {}
        for i, ts in enumerate(timesteps_o):
            if ts not in predictions_dict.keys():
                predictions_dict[ts] = dict()
            predictions_dict[ts][nodes[i]] = np.transpose(predictions_all[:, [i]], (1, 0, 2, 3))
        ...
        self.log.info(f"Epoch {epoch} Best Of 20: ADE: {ade} FDE: {fde}")
        print(f"Epoch {epoch} Best Of 20: ADE: {round(ade, 4)} FDE: {round(fde, 4)}")
    ...
    def _build_dir(self):
        ...
        # num_class = str(self.config.class_num)
        num_class = self.config.class_num
        self.eval_data_path = osp.join(self.config.data_dir, self.config.dataset + f"_test_{num_class}.pkl")  #########
        # print("> Directory built!")
        print("eval_dir: " + self.eval_data_path)
        self.log.info("eval_dir: " + self.eval_data_path)
        self.log.info("\n")
