...
class MultimodalGenerativeCVAE(object):
    def __init__(
        ...
        dynamic_class = SingleIntegrator
    ...
    def encode_node_history(self, mode, node_hist, first_history_indices):
        ...
        b_b = last_index_per_sequence.int().long()

        oout = outputs[torch.arange(first_history_indices.shape[0]), b_b]
    ...
    def encode_edge(
        ...
        last_index_per_sequence = -(first_history_indices + 1)
        ret = outputs[
            torch.arange(last_index_per_sequence.shape[0]), last_index_per_sequence.int().long()
        ]
        if self.hyperparams["dynamic_edges"] == "yes":
...
import torch
import numpy as np
from .dataset.preprocessing import get_timesteps_data, restore
